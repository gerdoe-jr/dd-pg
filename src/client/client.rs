...
use client_demo::{DemoRecorder, DemoVideoEncodeProperties, DemoViewer};
            ...
            if let Some(demo_player) = &mut self.demo_player {
                ...
                for ui_event in ui_events {
                    match ui_event {
                        ...
                        UiEvent::EncodeDemoToVideo { name } => {
                            self.demo_player = Some(DemoViewer::new(
                                &self.io,
                                &self.thread_pool,
                                name.as_ref(),
        ...
        if let Game::Active(game) = &mut self.game {
            ...
            for (local_player_id, local_player) in game.game_data.local_players.iter_mut() {
                ...
                let should_send_rates = !local_player
                    .sent_input_time
                    .is_some_and(|time| self.cur_time - time < time_per_tick);
                ...
                if send_by_input_change || (should_send_old_input && should_send_rates) {
                    ...
                    let player_input_chains = player_inputs
                        ...
                        .or_insert_with(|| game.player_inputs_chainable_pool.new());
                        ...
                        pred_inps.insert(
                            *id,
                            CharacterPredictionInput {
                                inp: tick_inp.inp,
                                diff,
                                intra_tick_ratio: intra_tick_time_to_ratio(
                                    game.game_data.intra_tick_time,
                                    ticks_per_second,
                                ),
                            },
