use accounts_shared::{
    account_server::{
        login::LoginError, login_token::LoginTokenError, result::AccountServerReqResult,
        sign::SignResponse,
    },
    client::{
        ...
        login::LoginRequest,
        login_token::{LoginTokenEmailRequest, LoginTokenSteamRequest},
...
pub trait SafeIo: Sync + Send {
    ...
    ) -> anyhow::Result<AccountServerReqResult<(), LoginTokenError>, HttpLikeError>;
    async fn request_login_steam_token(
        &self,
        data: LoginTokenSteamRequest,
    ) -> anyhow::Result<AccountServerReqResult<(), LoginTokenError>, HttpLikeError>;
    async fn request_login_email_token_with_secret_key(
        &self,
        data: LoginTokenEmailRequest,
    ) -> anyhow::Result<AccountServerReqResult<(), LoginTokenError>, HttpLikeError>;
    async fn request_login_steam_token_with_secret_key(
        &self,
        data: LoginTokenSteamRequest,
    ) -> anyhow::Result<AccountServerReqResult<(), LoginTokenError>, HttpLikeError>;
...
impl<'a> SafeIo for IoSafe<'a> {
    ...
    async fn request_login_steam_token(
        ...
        let res = self
            .io
            .request_login_steam_token(serde_json::to_string(&data)?.into_bytes())
            .await?;
        Self::des_from_vec(res)
    }
    async fn request_login_email_token_with_secret_key(
        ...
        let res = self
            .io
            .request_login_email_token_with_secret_key(serde_json::to_string(&data)?.into_bytes())
            .await?;
        Self::des_from_vec(res)
    }
    async fn request_login_steam_token_with_secret_key(
        ...
        let res = self
            .io
            .request_login_steam_token_with_secret_key(serde_json::to_string(&data)?.into_bytes())
            .await?;
        Self::des_from_vec(res)
    }
    async fn request_login(
