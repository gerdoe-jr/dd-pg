...
use accounts_shared::game_server::user_id::VerifyingKey;
use anyhow::anyhow;
...
impl CertsDownloader {
    pub async fn new(client: Arc<ClientHttpTokioFs>) -> anyhow::Result<Arc<Self>> {
        ...
        match certs_file {
            ...
            Err(_) => {
                ...
                let certs = account_client::certs::download_certs(client.as_ref()).await?;
    ...
    pub async fn download_task(&self) -> ! {
        loop {
            let invalid_in = self
                ...
                .map(|c| {
                    c.tbs_certificate
                        ...
                        .to_date_time()
                        .to_system_time()
                        .duration_since(SystemTime::now() + Duration::from_secs(7 * 24 * 60 * 60))
            ...
            if let Ok(certs) = account_client::certs::download_certs(self.client.as_ref()).await {
