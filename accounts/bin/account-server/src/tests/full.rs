...
async fn account_full_process() {
    let test = async move {
        ...
        let account_info = account_client::account_info::account_info(&*client).await?;
        assert!(account_info
            .credentials
            .iter()
        ...
        account_client::logout_all::logout_all(account_token_hex, &*client).await?;

        // signing should still work
        assert!(account_client::sign::sign(&*client).await.is_ok(),);
        ...
        .await?;
        let account_token_hex = account_token.lock().clone();
        account_client::delete::delete(account_token_hex, &*client).await?;
        ...
        .await?;
        let token_hex = token.lock().clone();
        account_client::link_credential::link_credential(account_token_hex, token_hex, &*client)
            .await?;
        ...
        // do actual login for client
        let token_hex = token.lock().clone();
        account_client::login::login(token_hex, &*client)
            .await?
            .1
            .write(&*client)
            .await?;
        ...
        let credential_auth_token_hex =
            ...
            .await?;
        account_client::login::login(credential_auth_token_hex, &*client)
            .await?
            .1
            .write(&*client)
            .await?;
        ...
        .await?;
        let credential_auth_token_hex = token.lock().clone();
        account_client::unlink_credential::unlink_credential(credential_auth_token_hex, &*client)
            .await?;
