...
use ip_limit::ip_deny_layer;
use login::{
    login_request,
    queries::{
        AccountIdFromEmail, AccountIdFromLastInsert, AccountIdFromSteam, CreateSession,
        InvalidateLoginToken, LinkAccountCredentialEmail, LinkAccountCredentialSteam,
        LoginTokenQry, TryCreateAccount,
    },
};
use login_token::{login_token_email, login_token_steam, queries::AddLoginToken};
...
struct LimiterSettings {
    login_tokens: LimiterValues,
    login_tokens_secret: LimiterValues,
    login: LimiterValues,
}
    ...
    fn default() -> Self {
        Self {
            ...
            login_tokens_secret: LimiterValues {
                ...
                // 5 request total, since the token system is handled by the
...
pub(crate) async fn prepare_statements(pool: &Pool<Any>) -> anyhow::Result<DbConnectionShared> {
    ...
    let login_token_statement = AddLoginToken::prepare(&mut connection).await?;
    ...
    let link_credentials_email_login_qry_statement =
        LinkAccountCredentialEmail::prepare(&mut connection).await?;
    let link_credentials_steam_login_qry_statement =
    ...
    Ok(DbConnectionShared {
        ...
        link_credentials_email_login_qry_statement,
        link_credentials_steam_login_qry_statement,
...
pub(crate) async fn prepare_http(
    ...
    let token_email = axum::Router::new()
        .route(
            ...
            axum::routing::post(move |payload: Json<_>| {
                login_token_email(shared_clone, pool_clone, false, payload)
            }),
        )
        .layer(layer(&settings.login_tokens)?);
    let shared_clone = shared.clone();
    let pool_clone = pool.clone();
    let token_steam = axum::Router::new()
        .route(
            "/token-steam",
            axum::routing::post(move |payload: Json<_>| {
                login_token_steam(shared_clone, pool_clone, false, payload)
            }),
        )
        .layer(layer(&settings.login_tokens)?);
    let shared_clone = shared.clone();
    ...
    let token_email_secret = axum::Router::new()
        .route(
            "/token-email-secret",
            axum::routing::post(move |payload: Json<_>| {
                login_token_email(shared_clone, pool_clone, true, payload)
            }),
        )
        .layer(layer(&settings.login_tokens_secret)?);
    let shared_clone = shared.clone();
    let pool_clone = pool.clone();
    let token_steam_secret = axum::Router::new()
        .route(
            "/token-steam-secret",
            axum::routing::post(move |payload: Json<_>| {
                login_token_steam(shared_clone, pool_clone, true, payload)
            }),
        )
        .layer(layer(&settings.login_tokens_secret)?);
    ...
    let shared_clone = shared.clone();
    let pool_clone = pool.clone();
    let login_by_token = axum::Router::new().route(
        "/by-token",
        axum::routing::post(move |payload: Json<_>| {
    ...
    // 4 KiB limit should be enough for all requests
    let request_size = DefaultBodyLimit::max(4096);
    let deny_list = Arc::new(RwLock::new(Default::default()));
    app = app
        .layer(request_size)
