...
use virtual_fs::{host_fs, mem_fs, AsyncReadExt, AsyncWriteExt, DirEntry, OpenOptionsConfig};
...
impl FileSystem {
    ...
    fn add_data_dir(scoped_file_systems: &mut Vec<ScopedDirFileSystem>) -> usize {
        scoped_file_systems.push(ScopedDirFileSystem {
            fs: Box::new(
                host_fs::FileSystem::new(tokio::runtime::Handle::current(), "data/").unwrap(),
            ),
            host_path: "data/".into(),
            mount_path: "".into(),
        });
    ...
    pub fn new(
        ...
        scoped_file_systems.push(ScopedDirFileSystem {
            fs: Box::new(
                host_fs::FileSystem::new(tokio::runtime::Handle::current(), config_dir.clone())
                    .unwrap(),
            ),
        ...
        if let Ok(exec_path) = std::env::current_dir() {
            scoped_file_systems.push(ScopedDirFileSystem {
                fs: Box::new(
                    host_fs::FileSystem::new(tokio::runtime::Handle::current(), exec_path.clone())
                        .unwrap(),
                ),
                host_path: exec_path,
                mount_path: "".into(),
