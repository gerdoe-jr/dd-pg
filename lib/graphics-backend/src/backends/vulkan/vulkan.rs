...
use graphics_types::{
    commands::{
        ...
        CommandIndicesForQuadsRequiredNotify, CommandMultiSampling, CommandRecreateBufferObject,
        CommandRender, CommandRenderQuadContainer, CommandRenderQuadContainerAsSpriteMultiple,
...
use super::{
    ...
    common::{tex_format_to_image_color_channel_count, texture_format_to_vulkan_format},
    ...
    vulkan_types::{
        DescriptorPoolType, DeviceDescriptorPools, EMemoryBlockUsage, RenderPassSubType,
...
impl VulkanBackend {
    ...
    fn new_command_group(
        ...
        if !self.current_command_group.cmds.is_empty() {
            ...
            let mut frame_g = self.frame.lock();
            let frame = &mut *frame_g;
            while self.current_command_group.render_pass_index
            ...
            frame.render.canvas_mode_mut(canvas_index).passes
                ...
                .render_pass_type = self.current_command_group.render_pass;
        }
    ...
    fn wait_frame(&mut self) -> anyhow::Result<()> {
            ...
            .is_err_and(|err| err == vk::Result::ERROR_OUT_OF_DATE_KHR)
        {
            ...
            queue_present_res.map_err(|err| anyhow!("Presenting graphics queue failed: {err}"))?
        };
