...
type OptionalFetchTasks = HashMap<u64, IoBatcherTask<Option<HashMap<String, DbType>>>>;
type FetchAllTasks = HashMap<u64, IoBatcherTask<Vec<HashMap<String, DbType>>>>;
...
pub struct WasmDatabaseLogicImpl {
    ...
    prepare_tasks: RefCell<HashMap<u64, IoBatcherTask<u64>>>,
    ...
    execute_tasks: RefCell<HashMap<u64, IoBatcherTask<u64>>>,
}
    ...
    fn prepare_statement(
        ...
        match tasks.get(&id) {
            ...
            None => {
                ...
                let task = self
                    .io_batcher
                    .spawn(async move { db.prepare_statement(&query_props, &driver_props).await });
                tasks.insert(id, task);
                None
            }
        }
...
impl WasmDatabaseLogic {
    ...
    pub fn get_wasm_logic_imports(
        ...
        fn prepare_statement(
            ...
            let res = logic_clone.prepare_statement(id, query_props, driver_props);
            write_result(instance.as_ref().unwrap(), &mut store, &res);
        }
        ...
        fn execute(
            ...
            let res = logic_clone.execute(id, unique_id, args);
            write_result(instance.as_ref().unwrap(), &mut store, &res);
        }
