...
pub fn render(ui: &mut egui::Ui, pipe: &mut UiRenderPipe<UserData>) {
    ui.with_layout(Layout::top_down(egui::Align::Min), |ui| {
        ...
        if !config.cl.spatial_chat.read_warning {
            ...
            egui_commonmark::CommonMarkViewer::new("spatial-chat-risks-and-warn").show(
                ui,
                &mut cache,
                EXPLAIN_SPATIAL_RISKS,
            );
            ...
            Grid::new("spatial-select-grid")
                ...
                .show(ui, |ui| {
                    ...
                    ui.add(Label::new("Sound from non-account users"))
                        .on_hover_ui(|ui| {
                            let mut cache = egui_commonmark::CommonMarkCache::default();
                            egui_commonmark::CommonMarkViewer::new("non-account-user-explain")
                                .show(ui, &mut cache, EXPLAIN_NON_ACCOUNT_USERS);
                        });
