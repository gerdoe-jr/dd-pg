...
impl RenderMap {
    ...
    fn get_physics_layer_texture<'a, L>(
        _layer: &MapLayerPhysicsSkeleton<L>,
        entities: &'a Entities,
            ...
            match sound.shape {
                SoundShape::Rect { size } => {
                    ...
                    let quad = StreamedQuad::default()
                        .from_pos_and_size(center, vec2::new(size.x.to_num(), size.y.to_num()))
        ...
        Q: Borrow<QuadLayerVisuals>,
    {
        ...
        if let Some(clipping) = &group_attr.clipping {
            // set clipping
            if !self.set_group_clipping(&mut state, center, camera.zoom, clipping) {
                return;
            }
        }
