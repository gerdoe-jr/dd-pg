...
pub trait GameStateInterface: GameStateCreate {
    ...
    /// The reason this function takes `old_snapshot` & `snapshot` is as follows:
    /// If for example your snapshot has a counter for non-linear (whatever) events,
    /// so that e.g. a teleport of a character would increase this counter. (Whatever your imagination can add here
    /// to make demo playback logically and smooth at the same time).
