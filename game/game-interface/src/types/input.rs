...
/// When a prediction tick happens, this is the object
/// the client passes in for a local predictable character input.
#[derive(Debug, Hiarc, Clone, Serialize, Deserialize)]
pub struct CharacterPredictionInput {
    /// The current character's input
    pub inp: CharacterInput,
    /// The difference in consumable inputs
    pub diff: CharacterInputConsumableDiff,
    /// When in the intra tick the above input changed.
    /// This can be useful to make the prediction of the charcter smoother.
    /// E.g.:
    /// ```norun
    /// Tick        0 --------------- Tick 1  
    ///                  ^ input to move right at 0.25 intra tick
    /// Tee-pos (0,0) --------------- Predicted tee-pos (1,0)
    ///                  <----------> in this range the tee has to do
    ///                               the full movement from non-redicted
    ///                               to predicted pos   
    /// ```
    /// If the input came in at 0.25 and the player wants to move to the right
    /// then the moves from non-predicted to predicted position in only 0.75 ticks
    /// (instead of the full tick).
    /// Note: that prediction here means future prediction, see the prediction code section
    /// of [`crate::interface::GameStateInterface`] for more explaination.
    pub intra_tick_ratio: f64,
}
