...
use client_http_fs::{cert_downloader::CertsDownloader, client::ClientHttpTokioFs, fs::Fs};
...
use shared_game::sql::account_info::AccountInfo;
...
enum GameServerDbAccount {
    Rename {
        ...
        rename_result: Result<NetworkReducedAsciiString<32>, String>,
    },
    Info {
        con_id: NetworkConnectionId,
        account_details: Result<account_info::AccountInfo, String>,
    },
    AutoLogin {
        user_id: UserId,
...
pub struct Server {
    ...
    db_requests: Vec<IoBatcherTask<GameServerDb>>,
    db_requests_helper: Vec<IoBatcherTask<GameServerDb>>,
...
impl Server {
    ...
    pub fn db_setup_task(
        ...
        io_batcher.spawn(async move {
                ...
                && config_db.connection_count != 0
            {
                ...
                let (accounts, account_info) = if config_db.enable_accounts {
                    account_game_server::setup::setup(&db.pool).await?;
                    Some((
                        account_game_server::prepare::prepare(&db.pool).await?,
                        AccountInfo::new(game_db.clone()).await?,
                    ))
                } else {
                    None
                }
                ...
                Ok((Some(db), game_db, accounts, account_info))
            } else {
                let game_db: Arc<dyn DbInterface> = Arc::new(DummyDb);
                Ok((None, game_db, None, None))
            }
        })
    }
    ...
    pub fn new(
        ...
        let account_certs_downloader = io.io_batcher.spawn(async move {
            if accounts_enabled {
        ...
        *shared_info.sock_addr.lock().unwrap() = Some(sock_addr);
        ...
        let (db, game_db, accounts, account_info) = task.get_storage()?;
        ...
        if let Some(player) = client {
            if player.players.contains_key(player_id) {
                match player_msg {
                    ...
                    ClientToServerPlayerMessage::JoinTeam { name, color } => {
                        self.game_server
                            .game
                            .client_command(player_id, ClientCommand::JoinStage { name, color });
                    }
                    ...
                    ClientToServerPlayerMessage::SwitchToFreeCam(mode) => {
                        self.game_server
                            .game
                            .client_command(player_id, ClientCommand::SwitchToFreeCam(mode));
                    }
        ...
        match game_msg {
            GameMessage::ClientToServer(client_to_server_msg) => {
                match client_to_server_msg {
                    ClientToServerMessage::Ready(ready_info) => {
                        ...
                        if let Some(client) = client {
                                ...
                                self.accounts.as_ref().zip(self.db.as_ref())
                            {
                                ...
                                self.db_requests.push(self.io.io_batcher.spawn(async move {
                                    let new_account_was_created =
                                        account_game_server::auto_login::auto_login(
                    ...
                    ClientToServerMessage::RconExec { name, args } => {
                        ...
                        if let Some(client) = self.clients.clients.get_mut(con_id) {
                            if !std::mem::replace(&mut client.requested_account_rename, true) {
                                    ...
                                    self.accounts.as_ref().zip(self.db.as_ref())
                                {
                                    ...
                                    let con_id = *con_id;
                                    self.db_requests.push(self.io.io_batcher.spawn(async move {
                                        let rename_res = account_game_server::rename::rename(
                                            account,
                                            &pool,
                                            &user_id,
                                            new_name.as_str(),
                                ...
                                self.network.send_unordered_to(
                                    &GameMessage::ServerToClient(
                                        ServerToClientMessage::AccountRenameRes(Err(
                                            "cannot change name that often".to_string(),
                                        )),
                                    ),
                                    con_id,
                                );
                    ...
                    ClientToServerMessage::AccountRequestInfo => {
                        if let Some(client) = self.clients.clients.get_mut(con_id) {
                            if !std::mem::replace(&mut client.requested_account_details, true) {
                                ...
                                if let Some((account_info, account_id)) =
                                    self.account_info.as_ref().zip(user_id.account_id)
                                {
                        ...
                        if let Some(spatial_chat) = &mut self.game_server.spatial_world {
                                ...
                                .and_then(|c| c.players.front().map(|(id, _)| (id, &c.auth)))
                            {
                                let account_server_public_keys = self
                                    ...
                                    .map(|c| c.public_keys())
                                    .unwrap_or_default();
                                let player_unique_id = Self::user_id_to_player_unique_id(
                                    &Self::user_id(&account_server_public_keys, auth),
                                );
    ...
    pub fn register(&mut self) {
        ...
        let mut register_info = ServerBrowserInfo {
            name: self.config_game.sv.name.clone(),
            game_type: self.game_server.game.info.mod_name.to_string(),
            ...
            players: characters
                ...
                .map(|(_, c)| ServerBrowserPlayer {
                    score: c.browser_score.to_string(),
                    name: c.info.name.to_string(),
                    clan: c.info.clan.to_string(),
                    flag: c.info.flag.to_string(),
    ...
    pub fn run(&mut self) {
        ...
        while self.is_open.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            if let Some(spatial_world) = &mut self.game_server.spatial_world {
                spatial_world.update(&mut self.network);
            }
                            ...
                            GameServerDb::Account(ev) => match ev {
                                GameServerDbAccount::Rename {
                                    ...
                                    if self.clients.clients.contains_key(&con_id) {
                                        self.network.send_unordered_to(
                                            &GameMessage::ServerToClient(
                                                ServerToClientMessage::AccountRenameRes(
                                                    rename_result,
                                                ),
                                            ),
                                            &con_id,
                                ...
                                GameServerDbAccount::Info {
                                    ...
                                    if self.clients.clients.contains_key(&con_id) {
                                        self.network.send_unordered_to(
                                            &GameMessage::ServerToClient(
                                                ServerToClientMessage::AccountDetails(
                                                    account_details,
                                                ),
                                            ),
                                            &con_id,
