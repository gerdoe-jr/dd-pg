pub mod state {
    ...
    impl GameState {
        ...
        fn stage_ctf_flags(
            ...
            let mut collect_flags = |flags: &Flags, pred_flags: &Flags| {
                res.extend(flags.iter().filter_map(|(&id, flag)| {
                    ...
                    let (pos, pred_flag) =
                        if pred_flag.core.non_linear_event != flag.core.non_linear_event {
                            ...
                            let pos = flag
                                ...
                                .and_then(|id| {
                                    stage
                                        ...
                                        .characters
                                        .get(&id)
                                        .zip(pred_stage.world.characters.get(&id))
                                })
                                .map(|(char, pred_char)| {
                                    self.stage_character_render_info(
                                        stage, pred_stage, char, pred_char, ratio,
                                    )
                                    .lerped_pos
                                })
                                .unwrap_or(flag.core.pos);
                            (pos, flag)
                        } else {
                            (Flag::lerped_pos(flag, pred_flag, ratio) / 32.0, pred_flag)
                        };
                    Some((
                        id,
                        FlagRenderInfo {
        ...
        fn stage_character_render_info(
            &self,
            stage: &GameStage,
            pred_stage: &GameStage,
            character: &Character,
            pred_character: &Character,
            intra_tick_ratio: f64,
        ) -> CharacterRenderInfo {
            CharacterRenderInfo {
                lerped_pos: character::lerp_core_pos(character, pred_character, intra_tick_ratio)
                    / 32.0,
                lerped_vel: character::lerp_core_vel(character, pred_character, intra_tick_ratio)
                    / 32.0,
                lerped_hook_pos: {
                    ...
                    hooked_char
                        .and_then(|hooked_char_id| {
                            ...
                            let pred_hooked_char = pred_stage
                                ...
                                .or(hooked_char);
                            hooked_char
                                .zip(pred_hooked_char)
                                .map(|(character, pred_character)| {
                ...
                buffs: {
                    ...
                    buffs.extend(character.reusable_core.buffs.iter().map(
                        |(buff, _)| match buff {
                            CharacterBuff::Ninja => (
                                CharacterBuff::Ninja,
                                CharacterBuffInfo {
                ...
                debuffs: {
                    ...
                    debuffs.extend(character.reusable_core.debuffs.iter().map(|(debuff, _)| {
                        match debuff {
    ...
    impl GameStateInterface for GameState {
        ...
        fn client_command(&mut self, player_id: &GameEntityId, cmd: ClientCommand) {
            match cmd {
                ...
                ClientCommand::SwitchToFreeCam(_) => {
