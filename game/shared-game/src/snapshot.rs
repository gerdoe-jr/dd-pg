pub mod snapshot {
    ...
    use pool::{
        datatypes::{PoolLinkedHashMap, PoolString, PoolVec, StringPool},
        pool::Pool,
    };
    ...
    impl SnapshotManager {
            ...
            game.game.stages.values().for_each(|stage| {
                ...
                stages.insert(
                    ...
                    SnapshotStage {
                        ...
                        stage_name: self.snapshot_pool.string_pool.new_str(&stage.stage_name),
                        ...
                        res.local_players.insert(
                            *id,
                            SnapshotLocalPlayer {
                                ...
                                input_cam_mode: PlayerCameraMode::Default,
                            },
                        );
                    } else if let Some(p) = game.game.no_char_players.player(id) {
                        res.local_players.insert(
                            *id,
                            SnapshotLocalPlayer {
                                is_dummy: p.player_info.is_dummy,
                                input_cam_mode: match p.no_char_type {
                                    NoCharPlayerType::Spectator => PlayerCameraMode::Free,
                                    NoCharPlayerType::Dead { died_at_pos, .. } => {
                                        PlayerCameraMode::LockedTo(died_at_pos / 32.0)
                                    }
                                },
                            },
                        );
            ...
            for (_, no_char_player) in no_char_players.drain() {
                res.no_char_players.insert(
                    ...
                    SnapshotNoCharPlayer {
                        ...
                        network_stats: no_char_player.network_stats,
                    },
                );
            }
            ...
            snap_stages.drain().for_each(|(snap_stage_id, snap_stage)| {
                ...
                if !stages.contains_key(&snap_stage_id) {
                    GameState::insert_new_stage(
                        stages,
                        snap_stage_id,
                        snap_stage.stage_name.to_string(),
                ...
                match_manager.game_match.state = snap_stage.match_manager.state;
        ...
        pub fn build_from_snapshot(
            ...
            write_game_state
                ...
                .retain_with_order(hi_closure!(
                    ...
                    |id: &GameEntityId, _: &mut NoCharPlayer| -> bool {
                        no_char_players.contains_key(id)
                    }
                ));
            ...
            no_char_players.drain().for_each(|(id, p)| {
                ...
                // sort
                write_game_state.game.no_char_players.move_to_back(&id);
            });
