...
pub mod character {
    ...
    use super::{
        core::character_core::{Core, CorePipe, CoreReusable, PHYSICAL_SIZE},
    ...
    use crate::{
        ...
        simulation_pipe::simulation_pipe::{
            SimulationEntityEvents, SimulationEventWorldEntityType, SimulationPipeCharacter,
        },
    ...
    pub struct CharacterCore {
        ...
        /// When the input arrived.. only for prediction
        pub(crate) input_intra_tick_ratio: f64,
    ...
    impl Character {
        ...
        pub fn is_friendly_fire_side(
            _characters: &mut Characters,
            _self_char_id: &GameEntityId,
            _other_side: MatchSide,
        ) -> bool {
            false // TODO
        }
        ...
        fn create_hammer_hit(entity_events: &mut Vec<CharacterEvent>, pos: &vec2) {
            entity_events.push(CharacterEvent::Effect {
                pos: *pos / 32.0,
        ...
        pub fn take_damage(
            ...
            let killer_id = match &from {
                ...
                DamageTypes::FromEntityInSide(side) => {
                    if Self::is_friendly_fire_side(characters, self_char_id, *side) {
                        return CharacterDamageResult::None;
                    }
                    None
                }
            ...
            if let (CharacterDamageResult::Death, Some(killer)) =
                (&res, killer_id.and_then(|id| characters.get_mut(&id)))
            {
        ...
        fn fire_weapon(
            ...
            let full_auto = core.active_weapon == WeaponType::Grenade
                    ...
                    pipe.characters.for_other_characters_in_range(
                        ...
                        &mut |char, removed_characters| {
                            ...
                            if length(&(*char.pos.pos() - proj_start_pos)) > 0.0 {
                                ...
                                Self::create_hammer_hit(&mut self.entity_events, &proj_start_pos);
                            }
