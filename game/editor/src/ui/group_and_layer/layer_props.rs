    ...
    let window_res = match attr_mode {
                ...
                let res = window.show(ui.ctx(), |ui| {
                    egui::Grid::new("design group attr grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            if ui
                                .add(
                                    egui::Button::new("Image selection").selected(
                                        layer_editor.image_2d_array_selection_open.is_some(),
                                    ),
                                )
                                .clicked()
                            {
        ...
        LayerAttrMode::Physics => {
            if main_frame_only {
                ...
                let res = window.show(ui.ctx(), |ui| {
                    let res = ui.label(
                        "Physics layers have no properties. Look in the physics group instead.",
                    );
