...
use accounts_shared::{
    account_server::{errors::AccountServerRequestError, login_token},
    client::account_data::AccountDataForClient,
};
...
use client_http_fs::{
    ...
    fs::Fs,
    profiles::{Certificate, LoginTokenResult, Profiles, ProfilesLoading},
};
...
impl ProfilesInterface for Accounts {
    async fn login_email_token(
        ...
        match self.0.login_email_token(email, secret_token).await {
            Ok(_) => Ok(()),
            Err(err) => match err {
                LoginTokenResult::AccountServerRequstError(err) => match err {
                    AccountServerRequestError::LogicError(err) => match err {
                        login_token::LoginTokenError::WebValidationProcessNeeded { url } => {
                            Err(LoginTokenError::WebValidationProcessNeeded { url })
                        }
                    },
                    _ => Err(LoginTokenError::Other(err.into())),
                },
                _ => Err(LoginTokenError::Other(err.into())),
            },
        }
