...
mod test {
    ...
    fn compression_tests_for_map(map_name: &str) {
        ...
        fn compression_of_group(groups_encoded: Vec<u8>, benchmark: &Benchmark) {
            ...
            // TODO: lzma seems kinda broken on windows with mingw
            let len_lzma_default = 0;
            let len_lzma_best = 0;
            /*let mut lzma_enc = lzma::LzmaWriter::new_compressor(&mut writer, 6).unwrap();
            lzma_enc.write_all(groups_encoded.as_slice()).unwrap();
            lzma_enc.finish().unwrap();
            let len_lzma_default = writer.len();
            writer.clear();
            benchmark.bench("lzma default");

            let mut lzma_enc = lzma::LzmaWriter::new_compressor(&mut writer, 8).unwrap();
            lzma_enc.write_all(groups_encoded.as_slice()).unwrap();
            lzma_enc.finish().unwrap();
            let len_lzma_best = writer.len();
            benchmark.bench("lzma best");
            /*let writer_clone = writer.clone();
            let mut lzma_dec = lzma::LzmaReader::new_decompressor(writer_clone.as_slice()).unwrap();
            lzma_dec.read_to_end(&mut writer).unwrap();*/
            writer.clear();
